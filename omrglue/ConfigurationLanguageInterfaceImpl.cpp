...
bool
MM_ConfigurationLanguageInterfaceImpl::initializeSizeClasses(MM_EnvironmentBase* env)
{
#if defined(OMR_GC_SEGREGATED_HEAP)
	OMR_VM* omrVM = env->getOmrVM();
	omrVM->_sizeClasses = (OMR_SizeClasses*)env->getForge()->allocate(sizeof(OMR_SizeClasses), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (NULL == omrVM->_sizeClasses) {
		return false;
	}
...
uintptr_t
MM_ConfigurationLanguageInterfaceImpl::getEnvPoolNumElements(void)
{
	/* Ruby at a minimum will have a main thread, a timer thread, and the GC threads,
	 * we should leave at least some buffer room for user threads to avoid malloc penalty. */
#define RUBY_THREAD_MIN_COUNT 4
	MM_GCExtensionsBase *extensions = MM_GCExtensionsBase::getExtensions(GET_VM()->_omrVM);
	return extensions->gcThreadCount + RUBY_THREAD_MIN_COUNT;
}
