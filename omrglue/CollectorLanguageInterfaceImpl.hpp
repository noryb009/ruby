...
class MarkingCompleteRubyStats;
class MM_EnvironmentStandard;
class MM_ForwardedHeader;
class MM_MarkingScheme;
class MM_MemorySubSpaceSemiSpaceBase;
...
class MM_CollectorLanguageInterfaceImpl : public MM_CollectorLanguageInterface {
	typedef void (MM_CollectorLanguageInterfaceImpl::*freeObjFunction)(MM_EnvironmentBase * env, VALUE object);

private:
	J9HashTable *_pendingWeakReferenceTable;
	J9HashTable *_pendingFinalizableTable;
...
protected:
	...
	MM_CollectorLanguageInterfaceImpl(OMR_VM *omrVM)
		...
		,_pendingWeakReferenceTable(NULL)
		,_pendingFinalizableTable(NULL)
		,_omrVM(omrVM)
...
public:
	...
	virtual void detachVMThread(OMR_VM *omrVM, OMR_VMThread *omrThread, uintptr_t reason);
	...
	virtual void parallelGlobalGC_clearClassLoadersScannedFlag(MM_EnvironmentBase *env) {}
	...
	virtual void parallelGlobalGC_setMarkingScheme(MM_EnvironmentBase *env, void *markingScheme)
	{
...
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */
	virtual void markingScheme_initialize(MM_EnvironmentBase *env) {}
...
#endif /* OMR_GC_MODRON_COMPACTION */
	MMINLINE bool
	isValidObject(omrobjectptr_t omrObject)
	{
		return _extensions->getObjectMap()->isValidObject(omrObject);
	}

	virtual bool
	objectAllocationNotify(MM_EnvironmentBase * env, omrobjectptr_t omrObject)
	{
		return _extensions->getObjectMap()->markValidObject(env, omrObject);
	}

	MMINLINE bool
	markValidObject(MM_EnvironmentBase * env, omrobjectptr_t omrObject)
	{
		return _extensions->getObjectMap()->inlineMarkValidObjectNoAtomic(env, omrObject);
	}

	MMINLINE bool
	markValidRubyObject(MM_EnvironmentBase * env, languageobjectptr_t rubyObject)
	{
		omrobjectptr_t omrObject = _extensions->objectModel.convertLanguageObjectToOmrObject(rubyObject);
		return markValidObject(env, omrObject);
	}

	MMINLINE bool
	isValidRubyObject(languageobjectptr_t rubyObject)
	{
		omrobjectptr_t omrObject = _extensions->objectModel.convertLanguageObjectToOmrObject(rubyObject);
		return isValidObject(omrObject);
	}

	virtual omrobjectptr_t heapWalker_heapWalkerObjectSlotDo(omrobjectptr_t object);
	bool markingScheme_markObject(MM_EnvironmentBase* env, omrobjectptr_t object);
	bool markingScheme_markLeafObject(MM_EnvironmentBase * env, omrobjectptr_t object);
	bool markingScheme_isMarked(omrobjectptr_t object);

	MMINLINE MM_MarkingScheme * getMarkingScheme()
	{
...
protected:
private:
	void freeInstanceVariableTables(MM_EnvironmentBase *env);
	void freeFrozenStrings(MM_EnvironmentBase *env);
	void makeZombie(MM_EnvironmentBase * env, VALUE handle, void (*dfree)(void *), void *data);
	void makeIoZombie(MM_EnvironmentBase * env, VALUE handle);

	void freeClassOrModule(MM_EnvironmentBase * env, VALUE obj);
	void freeRegexp(MM_EnvironmentBase * env, VALUE obj);
	void freeData(MM_EnvironmentBase * env, VALUE obj);
	void freeFile(MM_EnvironmentBase * env, VALUE obj);
	void freeIClass(MM_EnvironmentBase * env, VALUE obj);
	void freeDynamicSymbol(MM_EnvironmentBase * env, VALUE obj);
	void doDefinedFinalization(MM_EnvironmentBase * env, VALUE obj);
	void callFinalizersForType(MM_EnvironmentBase * env, enum ruby_value_type type, freeObjFunction callback);
	void inlineCallFinalizersForType(MM_EnvironmentBase * env, enum ruby_value_type type, freeObjFunction freeObjFunction);
	void doDefinedFinalizationWrapper(MM_EnvironmentBase * env, VALUE obj);
	void callDefinedFinalizersForType(MM_EnvironmentBase * env, enum ruby_value_type type);
	void callFinalizersInParallelForData(MM_EnvironmentBase * env);
	void callFinalizers(MM_EnvironmentBase * env);

	uintptr_t postMarkRegion(GC_ObjectHeapIterator &objectIterator);
	void verifyHeapRegion(GC_ObjectHeapIterator &objectIterator);
	void postMarkCheck(MM_EnvironmentBase *env);
};
